...
def NaoGraduados_PivotTable_2(primeirosCbos,csv_PivotTableFinal):
    ...
    PivotTableFinal = pd.read_csv(
        csv_PivotTableFinal,
        dtype={'CBO-Domiciliar': str, '1.0': int, '2.0': int, '3.0': int, '4.0': int, '5.0': int}
    )
    ...
    for i in range(len(primeirosCbos)):
        ...
        # print(primeirosCbos,csv_PivotTableFinal)
        # exit(0)        
        Valor1 = PivotTableFinal.loc[PivotTableFinal['CBO-Domiciliar'] == str(primeirosCbos[i]), '1.0'].values[0]

        # print('Valor1:',Valor1)  
        # exit(0)       
        # Valor2 = PivotTableFinal.loc[PivotTableFinal.Ocupação_Código==int(str(primeirosCbos[i])), '2.0'].values[0]
        # Valor2 = PivotTableFinal.loc[PivotTableFinal['CBO-Domiciliar']==int(str(primeirosCbos[i])), '2.0'].values[0]
        Valor2 = PivotTableFinal.loc[PivotTableFinal['CBO-Domiciliar']== str(primeirosCbos[i]), '2.0'].values[0]
        # Valor3 = PivotTableFinal.loc[PivotTableFinal.Ocupação_Código==int(str(primeirosCbos[i])), '3.0'].values[0]
        # Valor3 = PivotTableFinal.loc[PivotTableFinal['CBO-Domiciliar']==int(str(primeirosCbos[i])), '3.0'].values[0]
        Valor3 = PivotTableFinal.loc[PivotTableFinal['CBO-Domiciliar']== str(primeirosCbos[i]), '3.0'].values[0]
        # Valor5 = PivotTableFinal.loc[PivotTableFinal.Ocupação_Código==int(str(primeirosCbos[i])), '5.0'].values[0]
        Valor5 = PivotTableFinal.loc[PivotTableFinal['CBO-Domiciliar']==str(primeirosCbos[i]), '5.0'].values[0]
        # Valor5 = PivotTableFinal
        ValorFinal= Valor1 + Valor2 + Valor3 + Valor5
        ...
        Valor4 = PivotTableFinal.loc[PivotTableFinal['CBO-Domiciliar']== str(primeirosCbos[i]), '4.0'].values[0]
...
def CBOs_Curso_v6(csv_estado,csv_CBO,curso_num,curso_nome,titulo10,titulo3,porcent_param,save_results_to):
    ...
    # X_CURSO_CBO = X[['Curso_Superior_Graduação_Código','Ocupação_Código']]
    X_CURSO_CBO = X[['Curso_Superior_Graduação_Código','CBO-Domiciliar']] #mudado  em 12/08/2025 ... troca de arquivo de CBOs
    # Filtrar e imprimir apenas os CBOs domiciliares que começam com zero à esquerda
    # cbo_zero = X_CURSO_CBO[X_CURSO_CBO['CBO-Domiciliar'].astype(str).str.startswith('0')]
    # print(cbo_zero)
    # exit(0)
    ...
    # print("len(Ocupação_Código_temp)",len(Ocupação_Código_temp))
    # exit(0)    
    for i in range(len(X_CURSO_CBO)):         
        if (Ocupação_Código_temp[i][0] == '2') or (Ocupação_Código_temp[i][0] == '1'):
            Curso_Superior_Graduação_Código.append(X_CURSO_CBO.Curso_Superior_Graduação_Código[i])
            # Ocupação_Código.append(X_CURSO_CBO.Ocupação_Código[i])
            Ocupação_Código.append(X_CURSO_CBO['CBO-Domiciliar'][i])
    # print("len(Ocupação_Código)",len(Ocupação_Código))
    # exit(0)          
    # ...
    # print("len(Curso_Superior_Graduação_Código)",len(Curso_Superior_Graduação_Código))
    # exit(0)   
    X_CURSO_CBO_Filter=[]
    for i in range(len(Curso_Superior_Graduação_Código)):
      tupla=(Curso_Superior_Graduação_Código[i],Ocupação_Código[i])
      X_CURSO_CBO_Filter.append(tupla)
    # print("len(X_CURSO_CBO_Filter)",len(X_CURSO_CBO_Filter))
    # exit(0)  
    X_CURSO_CBO = pd.DataFrame(X_CURSO_CBO_Filter)
    dict = {0:"Curso_Superior_Graduação_Código",
    #1:"Ocupação_Código",
    ...
    # dir = X_CURSO_CBO.index[X_CURSO_CBO['Curso_Superior_Graduação_Código'] == curso_num].tolist()
    # dir = X_CURSO_CBO.index[X_CURSO_CBO['Curso_Superior_Graduação_Código'] == curso_num].tolist()
    dir = X_CURSO_CBO.index[X_CURSO_CBO['Curso_Superior_Graduação_Código'].astype(str) == str(int(curso_num))].tolist()
    # print(X_CURSO_CBO['Curso_Superior_Graduação_Código'])
    # print("curso_num",type(curso_num), str(int(curso_num)))
    # print("len(dir)",len(dir))
    # exit(0)
    if(len(dir)>=1):
        Curso_dir = []
...
# # Cursos por CBO 28/04/2024
# # Achar CBOs por Curso
# def CBOs_Curso_v6_CBO2(csv_estado,csv_CBO,curso_num,curso_nome,titulo10,titulo3,porcent_param,save_results_to):
#     X = pd.read_csv(csv_estado)
#     # CBO = pd.read_csv(csv_CBO, dtype ='str')
#     # CBO = pd.read_csv('documentacao/CBO_CSV.csv', dtype ='str')
#     CBO = pd.read_csv('documentacao/cbo2002_subgrupo_principal.csv', dtype ='str')
#     # CBO_aux = pd.read_csv('documentacao/CBO_CSV_TabelaAuxiliar.csv', dtype ='str')
#     # CBO_aux = pd.read_csv('documentacao/cbo2002_subgrupo.csv', dtype ='str')
#     CBO_aux = pd.read_csv('documentacao/cbo2002_subgrupo_principal.csv', dtype ='str')
#     # CBO = CBO.drop(columns=['Unnamed: 0']) #comentado  em 12/08/2025 ... troca de arquivo de CBOs
...
#             # mas pega o 'Cod_CBO' correspondente ao 'Cod_Dom' filtrado
#             # CBO_aux_filtrado = CBO_aux[CBO_aux['Cod_Dom'].astype(str).isin(cbo_indices)][['Cod_Dom', 'Nome_CBO']]
#             CBO_aux_filtrado = CBO[CBO['Cod_CBO'].astype(str).isin(cbo_indices)]
#             # print("CBO_aux_filtrado")
#             # print(CBO_aux_filtrado)
#             # Filtra o DataFrame CBO_aux para manter apenas os CBOs presentes em cbo_indices, 
#             # mas pega o 'Cod_CBO' correspondente ao 'Cod_Dom' filtrado
...
#             # Para guardar os CBOs que estão em A_cbo_10 e não estão em CBO['Cod_CBO']:
#             cbo_nao_encontrados = [cbo for cbo in cbo_indices if cbo not in set(CBO['Cod_CBO'].astype(str))]
#             cbo_ainda_nao_encontrados = [cbo for cbo in cbo_indices if cbo not in set(CBO['Cod_CBO'].astype(str)) and cbo not in set(CBO_aux['Cod_CBO'].astype(str))] #--------------------
#             # Agora cbo_nao_encontrados contém os códigos que não existem em CBO['Cod_CBO']
...
def CBOs_Curso_v6_CBO2(csv_estado,csv_CBO,curso_num,curso_nome,titulo10,titulo3,porcent_param,save_results_to):
    # X = pd.read_csv(csv_estado)
    # Lê o arquivo, mantendo 'CBO-Domiciliar' como string e os outros campos como padrão
    X = pd.read_csv(csv_estado, dtype={'CBO-Domiciliar': str})
    ...
    if(len(dir)>=1):
        ...
        # if(len(A_cbo_10>=1)):
            ...
            # Para encontrar todos os CBOs que estão em A_cbo_10 e também em CBO['Cod_CBO'] ou CBO_aux['Cod_CBO']:
            # cbo_indices = [str(int(float(idx))) for idx in A_cbo_10.index]
            cbo_indices = [str(idx) for idx in A_cbo_10.index]
            # Filtra o DataFrame CBO para manter apenas os CBOs presentes em cbo_indices
            CBO_filtrado = CBO[CBO['Cod_CBO'].astype(str).isin(cbo_indices)]
            # print("CBO_filtrado")
            # print(CBO_filtrado)
            ...
            if (len(A_cbo_10)>=1): #if (len(A_cbo_10)>1): 16/08/2025
                # A_cbo_10['Cod_CBO'] = A_cbo_10['Cod_CBO'].astype("float").astype('str')
                # A_cbo_10['CBO_Nome'] = A_cbo_10['Cod_CBO'].str.cat(A_cbo_10['Nome'], sep =" ")
                # Salvar Cod_CBO como string, mantendo zeros à esquerda # 07/10/2025
                A_cbo_10['Cod_CBO'] = A_cbo_10['Cod_CBO'].astype(str)
                A_cbo_10['CBO_Nome'] = A_cbo_10['Cod_CBO'].str.cat(A_cbo_10['Nome'], sep =" ")
                # print("A_cbo_10...")
                # print(A_cbo_10) #16/08/2025
                # exit(0) #30/07/2025
                ...
                if (len(A_cbo_10)<1):
                    ...
                    for i in range(len(A_cbo_10)):
                        # primeiros.append(str(int(float(A_cbo_10['Cod_CBO'].iloc[i]))))
                        primeiros.append(str(A_cbo_10['Cod_CBO'].iloc[i]))
                        # primeiros.append(int(float(A_cbo_10.Cod_CBO[i])))
                #...
                # print("primeiros",primeiros) #07/10/2025
                #Deletar coluna
                del A_cbo_10["Cod_CBO"]
                del A_cbo_10["Nome"]
                # A_cbo_10
...
# Cursos por CBO 28/04/2024
# Achar CBOs por Curso
def CBOs_Curso_v6_CBO3(csv_estado,csv_CBO,curso_num,curso_nome,titulo10,titulo3,porcent_param,save_results_to):
    # X = pd.read_csv(csv_estado)
    X = pd.read_csv(csv_estado, dtype={'CBO-Domiciliar': str})
    # Verifique os valores únicos para depuração
    # print("Valores únicos em 'CBO-Domiciliar':", sorted(X['CBO-Domiciliar'].unique()))
    # Remova espaços em branco e preencha NaN com string vazia para evitar erros
    # X['CBO-Domiciliar'] = X['CBO-Domiciliar'].fillna('').astype(str).str.strip()
    # Filtrar e imprimir apenas os registros onde 'CBO-Domiciliar' começa com zero à esquerda
    # registros_zero_esquerda = X[X['CBO-Domiciliar'].str.startswith('0')]
    # print("registros_zero_esquerda:")
    # print(registros_zero_esquerda.sort_values(by='CBO-Domiciliar', ascending=True))
    # CBO = pd.read_csv(csv_CBO, dtype ='str')
    # CBO = pd.read_csv('documentacao/CBO_CSV.csv', dtype ='str')
    CBO = pd.read_csv('documentacao/cbo2002_subgrupo.csv', dtype ='str')
    # CBO_aux = pd.read_csv('documentacao/CBO_CSV_TabelaAuxiliar.csv', dtype ='str')
    # CBO_aux = pd.read_csv('documentacao/cbo2002_familia.csv', dtype ='str')
        # CBO_aux = pd.read_csv('documentacao/CBO_CSV_TabelaAuxiliar.csv', dtype ='str')
    CBO_aux = pd.read_csv('documentacao/cbo2002_subgrupo.csv', dtype ='str')
    # CBO = CBO.drop(columns=['Unnamed: 0']) #comentado  em 12/08/2025 ... troca de arquivo de CBOs
    # Imprimir CBO-Domiciliar do CBO_aux que começam com zero à esquerda
    # cbo_dom_zero = CBO_aux[CBO_aux['Cod_CBO'].astype(str).str.startswith('0')]
    # print("CBO_aux ...")
    # print(cbo_dom_zero)
    # print("")
    #Cbos -> Cursos ... criar um novo dataframe somente com cursos e cbos para facilitar
    ...
    if(len(dir)>=1):
        ...
        for i in range(len(dir)):
            ...
            Cbo_dir.append(cbo)
        # print("Curso_dir",Curso_dir)  
        # Imprimir somente os valores da coluna Cbo_dir que começam com zero à esquerda
        # cbo_zero = [cbo for cbo in Cbo_dir if str(cbo).startswith('0')]
        # print("Cbo_dir começando com zero ...")
        # print(cbo_zero)
        resultados_dir=[]
        for i in range(len(Curso_dir)):
            tupla=(Curso_dir[i],Cbo_dir[i])
        ...
        Curso_Cbo_dir = pd.DataFrame(resultados_dir)
        # print("Curso_Cbo_dir[0]", Curso_Cbo_dir[0])
        # # Imprimir somente os valores da coluna Cbo_dir que começam com zero
        # cbo_zero = [cbo for cbo in Cbo_dir if str(cbo).startswith('0')]
        # print("Cbo_dir começando com zero ...") #02/10/2025
        # print(cbo_zero) #02/10/2025
        dict = {0:"Curso", 1:"Cbo",}
        Curso_Cbo_dir.rename(columns=dict,inplace=True)
        # print("Curso_Cbo_dir",Curso_Cbo_dir)
        #CBOs Unicos ============================================================
        ...
        # if(len(A_cbo_10>=1)):
            ...
            # Para encontrar todos os CBOs que estão em A_cbo_10 e também em CBO['Cod_CBO'] ou CBO_aux['Cod_CBO']:
            # cbo_indices = [str(int(float(idx) for idx in A_cbo_10.index]
            cbo_indices = [str(idx) for idx in A_cbo_10.index] #07/10/2025
            # print("cbo_indices",cbo_indices) #02/10/2025
            # exit(0) #02/10/2025
            # Filtra o DataFrame CBO para manter apenas os CBOs presentes em cbo_indices
            CBO_filtrado = CBO[CBO['Cod_CBO'].astype(str).isin(cbo_indices)]
            # print("CBO_filtrado")
            ...
            # mas pega o 'Cod_CBO' correspondente ao 'Cod_Dom' filtrado
            # CBO_aux_filtrado = CBO_aux[CBO_aux['Cod_Dom'].astype(str).isin(cbo_indices)][['Cod_Dom', 'Nome_CBO']]
            CBO_aux_filtrado = CBO_aux[CBO_aux['Cod_CBO'].astype(str).isin(cbo_indices)][['Cod_CBO', 'Nome_CBO']] #02/10/2025
            # CBO_aux_filtrado = CBO[CBO['Cod_CBO'].astype(str).isin(cbo_indices)]
            # print("CBO_aux_filtrado")
            # print(CBO_aux_filtrado)
            # Filtra o DataFrame CBO_aux para manter apenas os CBOs presentes em cbo_indices, 
            ...
            # print("...")
            if (len(A_cbo_10)>=1): #if (len(A_cbo_10)>1): 16/08/2025
                # Salvar Cod_CBO como string, mantendo zeros à esquerda
                A_cbo_10['Cod_CBO'] = A_cbo_10['Cod_CBO'].astype(str)
                A_cbo_10['CBO_Nome'] = A_cbo_10['Cod_CBO'].str.cat(A_cbo_10['Nome'], sep =" ")
                # print("A_cbo_10...")
                # print(A_cbo_10) #16/08/2025
...
def Cursos_CBO_13_10(csv_estado,csv_CBO,csv_CURSOS,cbo_num,titulo3,NaoGraduados_qtdade,Graduados_Nao_Total,curso_num,curso_nome,primeirosCbos_Nome,i,porcent_param,save_results_to):
    ...
    if(len(A_Curso_11)>=1):
        ...
        A_Curso_11['Porcentagem'].iloc[qtdade]= round(float(NaoGraduados_qtdade)/float(Graduados_Nao_Total)* 100, 2) #26/09
        # print("A_Curso_11 ================================================================")
        # print(A_Curso_11)  
       ...
       return cbo_num,curso_nome,primeirosCbos_Nome,plt, cursos, nomes, porcentagens #retirar intensidade e string 10/09/2025
       
    # return cbo_num,curso_nome,primeirosCbos_Nome,intensidade,plt,string, cursos, nomes, porcentagens
    return cbo_num,curso_nome,primeirosCbos_Nome,plt, cursos, nomes, porcentagens #retirar intensidade 10/09/2025
...
def Ida_Volta(path,name,path1,name1, sx, modo):
    ...
    for f in range(0,89): # 89 = len(CursosCenso)
    # for f in range(0,15):    
    #   if (f==83):
    ...
    #   else:    
        ...
        if (primeirosCbos!=0)&(primeirosCbos!=0)&(Porcentagens!=0):
            #======================================================Achando a quantidade de Não-Graduados na PivotTable
            primeirosCbos,NaoGraduados,Graduados_Nao,Graduados = NaoGraduados_PivotTable_2(primeirosCbos, csv_PivotTableFinal)
            # print(primeirosCbos,NaoGraduados,Graduados_Nao,Graduados)
            # exit(0)
            #=====================================================Plotando os cursos de determinado cbo, sem função e salvando os plots ...
            Intensidade = []
            Porcentagens_vol = []
            ...
            for i in range (len(primeirosCbos)):
                ...
                if(int(float(primeirosCbos[i]))>=2000):
                    ...
                    # print(CBO,Curso,tresprimeirosCursos,fig,cursos_vol, nomes_vol, porcentagens_vol)
                    # exit(0)
                    #100%
    ...
    # df.astype(str).to_csv(save_results_to + '10Porcent_DF.csv', index=False)
    ...
    if modo == 10 and  sx == 'F':
        # usa os arquivos de 10%
        df.to_csv(save_results_to + '10Porcent_DF_Fem.csv', index=False, encoding='utf-8-sig', quoting=1)
    if modo == 10 and  sx == 'M':
        # usa os arquivos de 10%
...
def Tabela_Ida_Volta(path2,name2, sx, modo):
    ...
    if modo == 10 and  sx == 'F':    
        # usa os arquivos de 100%
        df ='graficos/10Porcent_DF_Fem.csv' 
        logging.info(" Gerando a tabelas idas e voltas ...") 

    if modo == 10 and  sx == 'M':                  
        # usa os arquivos de 100%
        df ='graficos/10Porcent_DF_Masc.csv'
        logging.info(" Gerando a tabelas idas e voltas ...")     
    ...
    if modo == 10 and  sx == 'F':   
        # usa os arquivos de 10%
        logging.info(" Concluindo a geração da tabela de idas e voltas ...") 
        df1.to_csv(save_results_to + '10Porcent_DF_Limpo_Fem.csv', index=False, encoding='utf-8-sig', quoting=1)
...
def Ida_Volta_CBO2_Curso2():
    ...
    csv_estado = os.path.join('processados/CSVs_ArquivoFinalGraduados/Brasil_Graduados_CBO_DiminuidoCBO2_Curso2.csv') # arquivo do censo do Brasil inteiro (somente graduados)
    ...
    # csv_PivotTableFinal =  os.path.join(path2[0],name2[0]) #Pivo Table Final
    # csv_PivotTableFinal =  os.path.join('processados/CSVs_PivotTableFinal/Brasil_PivotFinal_DiminuidaCBO2.csv') #Pivo Table Final
    csv_PivotTableFinal =  os.path.join('processados/CSVs_PivotTableFinal/Brasil_PivotFinal_CBO_DiminuidaCBO2.csv')
    ...
    #     f=f+1 # Pular o curso 88. curso_num: 863.0 curso_nome: SETOR MILITAR E DE DEFESA
    #  if (f==17):
    #      f=f+1 # Pular o curso 17, por enquanto... curso_num: 76.0 curso_nome: SERVIÇO SOCIAL # 02/10/2025
    #  if (f==19):
    #      f=f+1 # Pular o curso 19, por enquanto... curso_num: 84.0 curso_nome: SERVIÇOS DE TRANSPORTES # 02/10/2025  
    #  if (f==21):
    #      f=f+1 # Pular o curso 21, por enquanto... curso_num: 86.0 curso_nome: SERVIÇOS DE SEGURANÇA    # 02/10/2025  
    #   else:    
        curso_num= float(CursosCenso.curso_num.iloc[f])
        curso_nome= CursosCenso.curso_nome.iloc[f]
        titulo10= "Course " +  CursosCenso.curso_num.iloc[f] + ": " + CursosCenso.curso_nome.iloc[f] + " - 10% "
    ...
    df.to_csv(save_results_to + 'CBO2_10Porcent_DF_Curso02.csv', index=False, encoding='utf-8-sig', quoting=1)
    ...
    # df['CB'] = df['CB'].astype(str)
    # df.to_csv(save_results_to + 'CBO2_100Porcent_DF_Curso02.csv', index=False, encoding='utf-8-sig', quoting=1)
    return 
...
def Tabela_Ida_Volta_CBO2_Curso2():
    ...
    save_results_to = 'graficos/'
    # df1 = pd.read_csv('graficos/CBO2_100Porcent_DF.csv') 
    # df1 = pd.read_csv('graficos/CBO2_100Porcent_DF_Curso02.csv', dtype={'CB': str})  
    # save_results_to = 'graficos/'
    # df1 = pd.read_csv('graficos/CBO2_10Porcent_DF.csv')  
    # df1 = pd.read_csv('graficos/CBO2_10Porcent_DF_Curso02.csv')  
    df1 = pd.read_csv('graficos/CBO2_10Porcent_DF_Curso02.csv', dtype={'CB': str})  
    ...
    df1.to_csv(save_results_to + 'CBO2_10Porcent_DF_Limpo_Curso02.csv', index=False, encoding='utf-8-sig', quoting=1)
    ...
    # df1['CB'] = df1['CB'].astype(str)
    # df1.to_csv(save_results_to + 'CBO2_100Porcent_DF_Limpo_Curso02.csv', index=False, encoding='utf-8-sig', quoting=1)

    return

# def Ida_Volta_CBO3():
def Ida_Volta_CBO3(modo):

    logging.info(" Gerando as idas e voltas")   
    # csv_estado = os.path.join(path[0],name[0]) # arquivo do censo do Brasil inteiro (somente graduados)
    # csv_estado = os.path.join('processados/CSVs_ArquivoFinalGraduados/Brasil_Graduados_DiminuidoCBO3.csv') # arquivo do censo do Brasil inteiro (somente graduados)
    csv_estado = os.path.join('processados/CSVs_ArquivoFinalGraduados/Brasil_Graduados_CBO_DiminuidoCBO3.csv') # arquivo do censo do Brasil inteiro (somente graduados)
    ...
    csv_PivotTableFinal =  os.path.join('processados/CSVs_PivotTableFinal/Brasil_PivotFinal_CBO_DiminuidaCBO3.csv') #Pivo Table Final
...
def Tabela_Ida_Volta_CBO3(modo):
    # df =  os.path.join(path2[0],name2[1])
    # df ='graficos/10Porcent_DF.csv' 
    # df ='graficos/100Porcent_DF.csv' 
    # df1 = pd.read_csv(df)  
...
def Ida_Volta_CBO3_Curso2():
    ...
    csv_estado = os.path.join('processados/CSVs_ArquivoFinalGraduados/Brasil_Graduados_CBO_DiminuidoCBO3_Curso2.csv') # arquivo do censo do Brasil inteiro (somente graduados)
    # path1 = ibge_variable.paths(12)
    # name1 = ibge_variable.names(6)
    # csv_CBO = os.path.join(path1[0],name1[1]) # Tabela de CBOs
    # csv_CURSOS = os.path.join(path1[0],name1[2]) # Tabela de Cursos
    ...
    # Salvar garantindo que a coluna 'CB' seja string para preservar zeros à esquerda
    # df['CB'] = df['CB'].astype(str)
    # df.to_csv(save_results_to + 'CBO3_10Porcent_DF_Curso02.csv', index=False, encoding='utf-8-sig', quoting=1)
    # # Para o Excel, os zeros à esquerda podem ser perdidos ao abrir no Excel, mas o arquivo terá a coluna como texto
    # df.to_excel(save_results_to + 'CBO3_10Porcent_DF_Curso02.xlsx', index=False)
    # # 100%
    # df.to_csv(save_results_to + '100Porcent_DF.csv')
    # df.to_csv(save_results_to + 'CBO3_100Porcent_DF.csv')
    # df.to_csv(save_results_to + 'CBO3_100Porcent_DF_Curso02.csv')
    df['CB'] = df['CB'].astype(str)
    df.to_csv(save_results_to + 'CBO3_100Porcent_DF_Curso02.csv', index=False, encoding='utf-8-sig', quoting=1)
    return 
...
def Tabela_Ida_Volta_CBO3_Curso2():
    ...
    # df1 = pd.read_csv('graficos/CBO3_10Porcent_DF_Curso02.csv', dtype={'CB': str})
    #100%
    # save_results_to = 'graficos/'
    # df1 = pd.read_csv('graficos/CBO3_100Porcent_DF.csv')  
    # df1 = pd.read_csv('graficos/CBO3_100Porcent_DF_Curso02.csv')  
    df1 = pd.read_csv('graficos/CBO3_100Porcent_DF_Curso02.csv', dtype={'CB': str})
    ...
    # Salvar garantindo que a coluna 'CB' seja string para preservar zeros à esquerda
    # df1['CB'] = df1['CB'].astype(str)
    # df1.to_csv(save_results_to + 'CBO3_10Porcent_DF_Limpo_Curso02.csv', index=False, encoding='utf-8-sig', quoting=1)
    # # Para o Excel, os zeros à esquerda podem ser perdidos ao abrir no Excel, mas o arquivo terá a coluna como texto
    # df1.to_excel(save_results_to + 'CBO3_10Porcent_DF_Limpo_Curso02.xlsx', index=False)
    # # # 100%
    # df1.to_csv(save_results_to + '100Porcent_DF_Limpo.csv')
    # df1.to_excel(save_results_to + '100Porcent_DF_Limpo.xlsx')
    # df1.to_csv(save_results_to + 'CBO3_100Porcent_DF_Limpo.csv')
    ...
    # Salvar garantindo que a coluna 'CB' seja string para preservar zeros à esquerda
    df1['CB'] = df1['CB'].astype(str)
    df1.to_csv(save_results_to + 'CBO3_100Porcent_DF_Limpo_Curso02.csv', index=False, encoding='utf-8-sig', quoting=1)
    # Para o Excel, os zeros à esquerda podem ser perdidos ao abrir no Excel, mas o arquivo terá a coluna como texto
    df1.to_excel(save_results_to + 'CBO3_100Porcent_DF_Limpo_Curso02.xlsx', index=False)
    return